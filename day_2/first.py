def change_list(lista):
    Newlista=[]
    for l in lista:
        if (l=='A' or l=='X') : Newlista.append(1) #potrei usare anche switch
        elif (l=='B' or l=='Y') : Newlista.append(2)
        elif (l=='C' or l=='Z') : Newlista.append(3)
    return Newlista
...
def function(lista1,listamia,listasua):
    ...
    while(len(listamia)!=0 and len(listasua)!=0): #posso pure controllarne una sola
     ...
     if(pos>pos1) : Newlist.append(pos-pos1) #sta dopo di me
     elif (pos1>pos) : Newlist.append(3-pos1+pos) #sta prima di me. last pos-mia pos+sua pos-first po+1 (+1=salto per tornre indietro)
     elif (pos1==pos) : Newlist.append(0)         #-> 3-mia pos+sua pos
