...
def change_list1(lista):
    ...
    for l in lista:
        if (l == 'X'):
            Newlista.append(0)  # potrei usare anche switch
        elif (l == 'Y'):
            Newlista.append(3)
        elif (l == 'Z'):
            Newlista.append(6)
...
def function(lista1, listares, listasua):
    ...
    while (len(listares) != 0 and len(listasua) != 0):  # posso pure controllarne una sola
        ...
        if (listares[0] == 6):  # devo vincere, vado uno indietro
            if (pos == 2):
                Newlist.append(lista1[0])  # non posso andare indietro di uno
            else:
                Newlist.append(lista1[pos + 1])  # posso andare indietro di 1
